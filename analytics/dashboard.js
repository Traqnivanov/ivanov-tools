...
$('#logoutBtn').onclick=()=>signOut(auth);$('#mobileMenu').onclick=()=>$('#sidebar').classList.toggle('open');document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>{current=b.dataset.view;document.querySelectorAll('.nav button').forEach(x=>x.classList.toggle('active',x===b));$('#sidebar').classList.remove('open');render()});

// Restore saved period and site
const PERIOD_KEY='ivanov_dash_period';
const SITE_KEY='ivanov_dash_site';
try{
  let sp=$('#periodFilter'),ss=$('#siteFilter');
  let savedPeriod=localStorage.getItem(PERIOD_KEY);
  let savedSite=localStorage.getItem(SITE_KEY);
  if(savedPeriod&&sp)sp.value=savedPeriod;
  if(savedSite&&ss)ss.value=savedSite;
  let c=sp.value==='custom';
  $('#dateFrom').classList.toggle('hidden',!c);
  $('#dateTo').classList.toggle('hidden',!c);
}catch(e){}

$('#siteFilter').onchange=()=>{try{localStorage.setItem(SITE_KEY,$('#siteFilter').value)}catch(e){}render()};
$('#periodFilter').onchange=()=>{
  let v=$('#periodFilter').value;
  try{localStorage.setItem(PERIOD_KEY,v)}catch(e){}
  let c=v==='custom';
  $('#dateFrom').classList.toggle('hidden',!c);
  $('#dateTo').classList.toggle('hidden',!c);
  if(!c)load()
};$('#dateFrom').onchange=load;$('#dateTo').onchange=load;$('#refreshBtn').onclick=load;
