...
async function importLiveSummary(){
  ...
  source=source.replace("function openSources(label){document.querySelector('.nav button[data-view=\"sources\"]')?.click();","function openSources(label){if(label==='Google Ads'){document.querySelector('.nav [data-external-view=\"ads\"]')?.click();return}document.querySelector('.nav button[data-view=\"sources\"]')?.click();");
  source=source.replace('Сравнява сайтовете по сесии и клиентски сесии. Натисни сайт, за да филтрираш цялото табло само за него.','Сравнява сайтовете по сесии и клиентски сесии. Една сесия може да присъства в повече от един сайт, ако посетителят е минал между тях, затова редовете не се събират задължително до общия брой сесии. Натисни сайт, за да филтрираш цялото табло само за него.');
  if(source===before||!source.includes('window.IvanovPeriods.rangeFromControls()')||!source.includes("sessions.flatMap(s=>s.events.filter(e=>e.site===site))")||!source.includes('pageEvents.some(e=>BUSINESS.has(e.eventType))')||!source.includes('siteEvents.some(e=>BUSINESS.has(e.eventType))')||!source.includes("if(label==='Google Ads')")||!source.includes('редовете не се събират задължително до общия брой сесии'))throw new Error('summary_live_patch_not_applied');
