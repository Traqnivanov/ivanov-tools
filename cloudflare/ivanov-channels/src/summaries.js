...
function aggregate(events) {
  ...
  for (const sessionEvents of sessions.values()) {
    ...
    const end = ordered.findLast?.(event => event.eventType === 'session_end') || [...ordered].reverse().find(event => event.eventType === 'session_end');
    ...
    const geoEvent = [...ordered].reverse().find(event => event.eventType === 'session_geo') || ordered.find(event => event.city && event.city !== 'unknown');
    const geoKey = `${geoEvent?.city || 'unknown'}|${geoEvent?.country || 'unknown'}`;
    increment(summary.geo, geoKey);

    const seenPages = new Set();
    for (const event of ordered) {
      if (event.eventType !== 'page_view') continue;
      const page = keyedBucket(summary.pages, event.pagePath, {
        title: event.pageTitle || '', sessions: 0, pageViews: 0, interestedSessions: 0, clientSessions: 0,
      });
      page.pageViews++;
    ...
    if (String(firstPage?.source || '').toLowerCase() === 'google' && String(firstPage?.medium || '').toLowerCase() === 'cpc') {
      const key = [firstPage.campaign || '', firstPage.term || '', firstPage.content || ''].join('|');
      const campaign = keyedBucket(summary.campaigns, key, {
        campaign: firstPage.campaign || '', term: firstPage.term || '', content: firstPage.content || '', sessions: 0, clientSessions: 0,
      });
      campaign.sessions++;
...
async function eventsForRange(env, from, toExclusive) {
  ...
  `).bind(from, toExclusive).all();
  return (rows.results || []).map(normalizeRow);
}

function utcBoundaryForSofiaDay(day) {
  const [year, month, date] = day.split('-').map(Number);
  const probe = new Date(Date.UTC(year, month - 1, date, 12));
  const local = parts(probe.toISOString());
  ...
  return new Date(Date.UTC(year, month - 1, date) - offsetMs).toISOString();
}
...
async function summarizeRange(env, table, periodKey, fromDay, toDayExclusive) {
  const from = utcBoundaryForSofiaDay(fromDay);
  const to = utcBoundaryForSofiaDay(toDayExclusive);
  const events = await eventsForRange(env, from, to);
...
export async function refreshAnalyticsSummaries(env) {
  ...
  return { daily: { day: yesterday, ...daily }, monthly: { month, ...monthly } };
}
...
export async function listAnalyticsSummaries(env, url) {
  ...
  if (!validPeriodValue(period, from) || !validPeriodValue(period, to)) return { error: 'invalid_range', status: 400 };
  if (from > to) return { error: 'invalid_range', status: 400 };
